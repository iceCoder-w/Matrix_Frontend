      ...
      <div class="file-list">
        <ul v-for="file in fileList" :class="`file-${file.id}`" :key="file.name" class="file-item">
          ...
          <li class="file-operate">
            <a title="开始" @click="resume(file)"><i class="el-icon-upload2"/></a>
            <a title="暂停" @click="stop(file)"><i class="el-icon-minus"/></a>
            <a title="移除" @click="remove(file)"><i class="el-icon-delete"/></a>
          </li>
          <li class="progress"/>
        </ul>
...
<script>
import vueUpload from '@/components/WebUploader/upload'
import api from '@/api/matrix/file'
export default {
  components: {
    vueUpload
  },
...
.file-panel {
  ...
  .file-type[icon=text] {
    background: url(../../../../static/icon/text-icon.png);
  }
  .file-type[icon=video] {
    background: url(../../../../static/icon/video-icon.png);
  }
  .file-type[icon=image] {
    background: url(../../../../static/icon/image-icon.png);
  }
  .progress {
    position: absolute;
    top: 0;
